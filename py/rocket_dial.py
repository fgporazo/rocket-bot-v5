...
class RocketDial(commands.Cog):
    ...
    async def rd_report(self, ctx: commands.Context, *, reason: str = "No reason provided"):
        ...
        if reporter_count >= 3:
            return await ctx.send(
                "⚠️ You have reached the maximum number of reports (3)."
            )
        ...
        if reporter_channel:
            try:
                ...
                if last_msg:
                    parts = last_msg.content.split("|", maxsplit=3)
                    reporter_count = min(int(parts[2].strip().replace("x", "")) + 1, 3)
                    new_content = f"{reporter_id} | {reporter.display_name} | {reporter_count}x"
                    await last_msg.edit(content=new_content)
                else:
